...
agent.py::check_red_flags). No network/API key needed — that's
...
class _FakePool:
    """Stands in for agent._get_pool()'s ThreadedConnectionPool."""
...
def test_pipeline_fails_safe_to_emergency_when_db_is_unavailable(monkeypatch):
    ...
    monkeypatch.setattr(
        agent, "_get_pool", lambda: (_ for _ in ()).throw(RuntimeError("db down"))
    )
...
def test_pipeline_fails_safe_to_emergency_when_check_itself_fails(monkeypatch):
    ...
    monkeypatch.setattr(agent, "_get_pool", lambda: _FakePool(_FakeConn(None)))
