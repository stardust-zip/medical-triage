...
# ---------------------------------------------------------------------------
# POST /internal/triage/queue-resolved — called server-to-server by
# queue-service when a nurse resolves a queue item (Phase 5). Backfills
# triage_logs by the exact triage_log_id instead of queue-service writing
# into this service's table directly.
# ---------------------------------------------------------------------------


@app.post(
    "/internal/triage/queue-resolved",
    status_code=status.HTTP_200_OK,
    tags=["Internal"],
    include_in_schema=False,
)
async def queue_resolved(
    body: QueueResolvedRequest,
    _internal: None = Depends(require_internal_secret),
):
    async with db_connection(body.org_id) as conn:
        updated = await backfill_triage_resolution(
            conn, body.triage_log_id, body.approved_dept, body.resolution_type
        )
