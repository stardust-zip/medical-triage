...
def _fetch_top_red_flag_match(conn: Any, embedding_str: str) -> Any:
...
async def check_red_flags(
    ...
    try:
        ...
        row = await asyncio.to_thread(_fetch_top_red_flag_match, conn, embedding_str)
...
def _get_pool() -> ThreadedConnectionPool:
    """Lazily create the process-wide pooled-connection singleton."""
    global _pool
    if _pool is None:
...
def _get_semaphore() -> asyncio.Semaphore:
    """Gate checkouts so callers await a free slot instead of getconn() raising
    PoolError once the pool is full — kept separate from _get_pool() so it still
    works when tests stub that out."""
    global _pool_semaphore
    if _pool_semaphore is None:
        _pool_semaphore = asyncio.Semaphore(settings.DB_POOL_MAX_CONNECTIONS)
    return _pool_semaphore


def close_pool() -> None:
    """Release every pooled connection. Called from api.py's shutdown hook."""
    global _pool
...
def _set_org_context(conn: Any, org_id: str) -> None:
    """Set app.org_id for RLS (see db/init.sql). Pooled conn: must re-run every checkout."""
    with conn.cursor() as cur:
        cur.execute("SELECT set_config('app.org_id', %s, true)", (str(org_id),))


async def _checkout(org_id: str) -> Any:
    pool = _get_pool()
    await _get_semaphore().acquire()
    try:
        conn = await asyncio.to_thread(pool.getconn)
        await asyncio.to_thread(_set_org_context, conn, org_id)
        return conn
    except Exception:
...
async def _checkin(conn: Any) -> None:
    await asyncio.to_thread(_get_pool().putconn, conn)
    _get_semaphore().release()


@asynccontextmanager
async def _scoped_connection(conn: Any) -> AsyncGenerator[Any, None]:
    """Commit/rollback/release around an already-checked-out connection."""
    try:
        yield conn
...
async def db_connection(org_id: str) -> AsyncGenerator[Any, None]:
    """Pooled connection scoped to org_id. Raises on checkout failure — see optional_db_connection()."""
    conn = await _checkout(org_id)
    async with _scoped_connection(conn) as scoped:
        yield scoped


@asynccontextmanager
async def optional_db_connection(org_id: str) -> AsyncGenerator[Any, None]:
    """Like db_connection, but yields None instead of raising if the DB is unreachable."""
    try:
        conn = await _checkout(org_id)
    except Exception as exc:  # noqa: BLE001
...
async def insert_triage_log(
    ...
    """Insert a triage_logs row (final_dept/resolution_type backfilled later, see backfill_triage_resolution)."""
    log_id = str(uuid.uuid4())
    embedding_str = "[" + ",".join(str(x) for x in symptom_embedding) + "]"
    await asyncio.to_thread(
        _insert_triage_log_row,
        conn,
        log_id,
        raw_symptoms,
        embedding_str,
...
def _backfill_triage_resolution_row(
    conn: Any, triage_log_id: str, approved_dept: str, resolution_type: str
) -> bool:
    with conn.cursor() as cur:
        cur.execute(
            """
            UPDATE triage_logs
            SET    final_dept = %s, resolution_type = %s::triage_resolution
            WHERE  id = %s
            """,
            (approved_dept, resolution_type, triage_log_id),
        )
        return cur.rowcount > 0


async def backfill_triage_resolution(
    conn: Any, triage_log_id: str, approved_dept: str, resolution_type: str
) -> bool:
    """Backfill final_dept/resolution_type by exact id (called from POST /internal/triage/queue-resolved)."""
    return await asyncio.to_thread(
        _backfill_triage_resolution_row,
        conn,
        triage_log_id,
        approved_dept,
        resolution_type,
...
def _upsert_red_flag_row(conn: Any, keyword: str, embedding_str: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
...
async def seed_red_flags(conn: Any, keywords: list[str]) -> int:
    ...
    for keyword in keywords:
        try:
            ...
            embedding_str = "[" + ",".join(str(x) for x in embedding) + "]"
            await asyncio.to_thread(_upsert_red_flag_row, conn, keyword, embedding_str)
            count += 1
...
async def run_triage_pipeline(
    ...
    MAX_ITERATIONS = 5
    async with optional_db_connection(org_id) as conn:
